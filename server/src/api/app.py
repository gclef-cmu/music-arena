...
from src.api.utils import get_settings, generate_unique_id, get_timestamp, create_audio_metadata
...
class FastAPIApp:
    def __init__(self):
        ...
        self.app = FastAPI(title="Music Arena API", version=self.settings.get("version_backend", "0.1.0"))
        ...
        # Initialize clients
        # Use mock clients for testing if environment variable is set
        use_mock = os.environ.get("USE_MOCK_CLIENTS", "true").lower() == "true"
        
        if use_mock:
            logger.info("Using mock clients for Firebase and GCP")
            self.firebase_client = MockFirebaseClient()
            self.gcp_client = MockGCPClient()
        else:
            logger.info("Using real clients for Firebase and GCP")
    ...
    def setup_routes(self):
        ...
        async def health_check():
            ...
            return {"status": "healthy", "version": self.settings.get("version_backend", "0.1.0")}
        ...
        async def generate_audio_pair(request: AudioPairRequest, background_tasks: BackgroundTasks):
            ...
            try:
                ...
                # Check for errors
                if response_1.error:
                    raise HTTPException(status_code=500, detail=f"Error generating first audio: {response_1.error}")
                if response_2.error:
                    raise HTTPException(status_code=500, detail=f"Error generating second audio: {response_2.error}")
        ...
        @self.app.post("/upload_json")
        async def upload_json(request: Request, background_tasks: BackgroundTasks):
            """
            Upload a JSON document to Firebase and any associated audio files to Google Cloud Storage.
            """
            try:
                # Parse JSON data
                ...
                # If there's audio data in base64 format, upload it to GCS
                if "audioData" in data:
                    try:
                        ...
                        # Upload to GCS
                        bucket_name = self.settings["gcs_buckets"]["audio_files"]
                        gcs_path = f"uploads/{upload_id}.mp3"
                        
                        url = self.gcp_client.upload_file(bucket_name, audio_data, gcs_path)
                ...
                collection = self.settings["firebase_collections"]["audio_metadata"]
                doc_id = self.firebase_client.upload_data(collection, data)
                
                return {"status": "success", "uploadId": upload_id, "documentId": doc_id}
        ...
        async def upload_audio(
            ...
            """
            Upload an audio file to Google Cloud Storage and its metadata to Firebase.
            """
            try:
                # Generate a unique ID for this upload
                upload_id = generate_unique_id()
                ...
                url = self.gcp_client.upload_file(bucket_name, file_data, gcs_path)
                ...
                if metadata:
                    try:
                        ...
                        logger.warning("Invalid JSON metadata provided")
                ...
                # Upload metadata to Firebase
                collection = self.settings["firebase_collections"]["audio_metadata"]
                doc_id = self.firebase_client.upload_data(collection, doc_data)
