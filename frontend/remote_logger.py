'''
[Source] https://github.com/lmarena/FastChat-dev/blob/yonghyun/txt2music-dev/fastchat/serve/remote_logger.py
'''
...
def get_repochat_remote_logger():
    global _repochat_global_logger
    if _repochat_global_logger is None:
        if url := os.environ.get("REPOCHAT_REMOTE_LOGGER_URL"):
            logging.info(f"Repochat Remote logger enabled, sending data to {url}")
            _repochat_global_logger = RemoteLogger(url=url)
        else:
            _repochat_global_logger = EmptyLogger()
    return _repochat_global_logger
