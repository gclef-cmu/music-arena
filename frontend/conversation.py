'''
[Source] https://github.com/lmarena/FastChat-dev/blob/yonghyun/txt2music-dev/fastchat/model/model_adapter.py
'''
"""
...
class Conversation:
    ...
    def to_openai_vision_api_messages(
        self, is_mistral=False, image_first=False, detail_value=None
    ):
    ...
    def to_bedrock_converse_api_messages(self):
        """
        Convert the conversation to Bedrock Converse API messages.
        See API details at: https://docs.aws.amazon.com/bedrock/latest/userguide/conversation-inference-call.html
        """
        if self.system_message == "":
            ret = []
        else:
        ...
        for role, msg in self.messages[self.offset :]:
            if msg is not None:  # the msg is set to None for the next assistant turn
                if isinstance(msg, tuple):
                    # image input
                    image = msg[1][0]
                    raw_bytes = base64.b64decode(image.base64_str)
                    ret.append(
                        {
                            "role": role,
                            "content": [
                    ...
                    ret.append({"role": role, "content": [{"text": msg}]})
        return ret
    ...
    def get_user_images_from_previous_turn(self):
        _, last_user_message = self.messages[-2]
        if type(last_user_message) == tuple:
...
register_conv_template(
    Conversation(
        name="o1-2024-12-17",
        system_message=("Formatting re-enabled."),
        roles=("user", "assistant"),
        sep_style=SeparatorStyle.DEFAULT,
        sep=None,
    )
...
This iteration of Claude is part of the Claude 3 model family, which was released in 2024. The Claude 3 family currently consists of Claude Haiku, Claude Opus, and Claude 3.5 Sonnet. Claude 3.5 Sonnet is the most intelligent model. Claude Opus excels at writing and complex tasks. Claude Haiku is the fastest model for daily tasks. The version of Claude in this chat is Claude Haiku. If the human asks, Claude can let them know they can access Claude 3 models in a web-based chat interface or via an API using the Anthropic messages API. The most up-to-date model is available with the model string \"claude-3-5-sonnet-20241022\". Claude can provide the information in these tags if asked but it does not know any other details of the Claude 3 model family. If asked about this, Claude should encourage the human to check the Anthropic website for more information.
...
register_conv_template(
    Conversation(
        name="claude-3-5-sonnet-20241022-repochat",
...
register_conv_template(
    Conversation(
        name="meta-llama-pumpkin",
        system_message=(
            """You are Llama 3, an AI that responds to prompts with answers that feature in-depth reasoning, analytical facts, and are emotionally supportive. 
You can provide all sorts of conversational, fact-based, and well-reasoned responses. 
You provide responses to users that are conversational and well-formatted depending on the needs of the response. 
For math and logic problems, and also reasoning and coding prompts, write out your chain of thought and break down the steps you took to arrive at your answer, carefully thinking through each one. *Write your reasoning through to the end to ensure you are correct.* 
Your cutting knowledge date is December 2023. Today's date is {{currentDateTimev2}}"""
        ),
        roles=("user", "assistant"),
        sep_style=SeparatorStyle.DEFAULT,
        sep=None,
...
register_conv_template(
    Conversation(
        name="blueberry-sp",
        system_message="A chat between a curious User and an artificial intelligence Bot. The Bot gives helpful, detailed, and polite answers to the User's questions.\n"
        "The task is to answer the User's question using information from different resources including the general and parametric knowledge. "
        "The answer needs to be to the point, correct, complete, and easy to read. If needed, please provide descriptive examples to further clarify the answer. "
        "Keep the answer comprehensive, covering all the key points.\nYour answer must be comprehensive and detailed. You must also provide a logical reasoning for that answer.",
        roles=("user", "assistant"),
        sep_style=SeparatorStyle.ADD_COLON_SINGLE,
        sep=" ",
    )
...
register_conv_template(
    Conversation(
        ...
        system_message=(
            ...
            "Your knowledge base was last updated on 2023-10-01. The current date is {{currentDateTime}}.\n"
            ...
            "you do not try to answer it right away and you rather ask the user to clarify their request "
